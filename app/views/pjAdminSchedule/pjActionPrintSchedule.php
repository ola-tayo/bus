...
{ 
	...
	<div style="font-weight: bold; margin-bottom: 12px; overflow: hidden;">
		<div style="float: left;"><?php __('lblBusesOn')?>:&nbsp;<?php echo date($tpl['option_arr']['o_date_format'], strtotime($tpl['date']));?></div>
		<div style="float: right;"><?php __('lblCurrentDateTime')?>:&nbsp;<?php echo pjDateTime::formatDate(date('Y-m-d'), "Y-m-d", $tpl['option_arr']['o_date_format']);?> <?php echo pjDateTime::formatTime(date('H:i:s'), "H:i:s", $tpl['option_arr']['o_time_format']);?></div>
	</div>
			...
			if(count($tpl['bus_arr']) > 0)
			{
				...
				foreach($tpl['bus_arr'] as $v)
				{
					...
					<tr>
						...
						<td><?php echo pjDateTime::formatTime(date('H:i:s', strtotime($v['departure_time'])), "H:i:s", $tpl['option_arr']['o_time_format']);?></td>
						<td><?php echo pjDateTime::formatTime(date('H:i:s', strtotime($v['arrival_time'])), "H:i:s", $tpl['option_arr']['o_time_format']);?></td>
