...
{ 
	?>
	<div style="font-weight: bold; margin-top: 10px; margin-bottom: 12px;"><?php __('lblBus')?>:&nbsp;<?php echo $tpl['bus_arr']['route']?>, <?php echo pjDateTime::formatTime(date('H:i:s', strtotime($tpl['bus_arr']['departure_time'])), 'H:i:s', $tpl['option_arr']['o_time_format'])?> - <?php echo pjDateTime::formatTime(date('H:i:s', strtotime($tpl['bus_arr']['arrival_time'])), 'H:i:s', $tpl['option_arr']['o_time_format'])?> <?php __('lblOn');?> <?php echo date($tpl['option_arr']['o_date_format'], strtotime($controller->_get->toString('date')));?></div>
		...
		<thead>
			<tr>
				...
				foreach($tpl['location_arr'] as $k => $v)
				{
					...
					?><th style="width: 120px;"><?php echo pjSanitize::clean($v['location']);?><br/><?php echo !empty($v['departure_time']) ? pjDateTime::formatTime(date('H:i:s', strtotime($v['departure_time'])), "H:i:s", $tpl['option_arr']['o_time_format']) : '&nbsp;';?><?php echo $_str;?></th><?php
					if($k == 0)
					{
						$first_order = $v['order'];
					}
