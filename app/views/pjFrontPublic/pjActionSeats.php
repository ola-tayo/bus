		...
		<div class="panel-body pjBsBody">
			<div class="pjBsForm pjBsFormTickets">
				...
				if($tpl['status'] == 'OK')
				{
					...
					$selected_date = pjDateTime::formatDate($STORE['date'], $tpl['option_arr']['o_date_format']);
					$previous_date = pjDateTime::formatDate(date('Y-m-d', strtotime($selected_date . ' -1 day')), 'Y-m-d', $tpl['option_arr']['o_date_format']);
					$next_date = pjDateTime::formatDate(date('Y-m-d', strtotime($selected_date . ' +1 day')), 'Y-m-d', $tpl['option_arr']['o_date_format']);
					...
					if (isset($STORE['is_return']) && $STORE['is_return'] == 'T')
					{
						...
						$return_selected_date = pjDateTime::formatDate($STORE['return_date'], $tpl['option_arr']['o_date_format']);
						$return_previous_date = pjDateTime::formatDate(date('Y-m-d', strtotime($return_selected_date . ' -1 day')), 'Y-m-d', $tpl['option_arr']['o_date_format']);
						$return_next_date = pjDateTime::formatDate(date('Y-m-d', strtotime($return_selected_date . ' +1 day')), 'Y-m-d', $tpl['option_arr']['o_date_format']);
					}
					?>
					<form id="bsSelectSeatsForm_<?php echo $index;?>" action="" method="post">
						...
						<div class="pjBsFormBody">
							...
							{ 
								...
								<div class="panel panel-default pjBsSeats bsBusContainer">
													...
													<div class="col-lg-4 col-md-4 col-sm-4 col-xs-8">
														...
														<div id="pjBrTipClone_<?php echo $bus['id'];?>" style="display: none;">
															<ul class="list-unstyled pjBsListTicks">
																...
																{ 
																	?><li><span class="glyphicon glyphicon-ok" aria-hidden="true"></span><?php echo $location['content'] . " - " . (!empty($location['departure_time']) ? pjDateTime::formatTime(date('H:i:s', strtotime($location['departure_time'])), 'H:i:s', $tpl['option_arr']['o_time_format']) : pjDateTime::formatTime(date('H:i:s', strtotime($location['arrival_time'])), 'H:i:s', $tpl['option_arr']['o_time_format']));?></li><?php
																} 
																?>
																		...
																		<div class="col-sm-4 col-xs-12 ticket_select">
																			<div class="form-group">
																				...
																				<div class="input-group">
																					...
																					<span class="input-group-addon">x <?php echo pjCurrency::formatPrice($ticket['price']);?></span>
																				</div><!-- /.input-group -->
																			</div><!-- /.form-group -->
																		</div>
																		<?php
									...
									<div class="panel-body pjBsSeatsBody pjBsPickupSeatsBody" style="display: <?php echo isset($booked_data) && $booked_data['selected_ticket'] > 0 ? 'block' : 'none';?>;">
										...
										<div class="pjBsListSeats">
											<div class="pjBsChosenSeats" style="display:<?php echo isset($booked_data) && $booked_data['selected_ticket'] > 0 ? 'block' : 'none';?>;">
												<p><?php __('front_select');?> <strong id="bsSeats_<?php echo $index;?>"><?php echo isset($booked_data) ? ( empty($intersect) ? ( $booked_data['selected_ticket'] > 0 ? ($booked_data['selected_ticket'] != 1 ? ($booked_data['selected_ticket'] . ' ' . pjSanitize::clean(__('front_seats', true, false))) : ($booked_data['selected_ticket'] . ' ' . pjSanitize::clean(__('front_seat', true, false))) ) :null) :null): null;?></strong></p>
												...
												<dl class="dl-horizontal" style="display: <?php echo isset($booked_data) && $booked_data['selected_ticket'] > 0 ? 'inline-block' : 'none';?>;">
													...
													<dd id="bsSelectedSeatsLabel_<?php echo $index;?>"><?php echo isset($booked_data) ? ( empty($intersect) ? ($booked_data['selected_seats'] != '' ? join(", ", $tpl['selected_seat_arr']) : null) : null ) : null;?></dd>
												</dl><!-- /.dl-horizontal -->
											...
											<div id="bsMapContainer_<?php echo $index;?>" class="bsMapContainer" style="display:<?php echo isset($booked_data) && $booked_data['has_map'] == 'T' ? 'block' : 'none';?>;">
												<?php
									...
									<input type="hidden" id="bs_selected_tickets_<?php echo $index;?>" name="selected_ticket" value="<?php echo isset($booked_data) && $booked_data['selected_ticket'] > 0 ? $booked_data['selected_ticket'] : null;?>" data-map="<?php echo isset($booked_data) && $booked_data['selected_ticket'] > 0 ? (!empty($tpl['bus_type_arr']['seats_map']) ? 'T' : 'F') : 'F';?>"/>
									<input type="hidden" id="bs_selected_seats_<?php echo $index;?>" name="selected_seats" value="<?php echo isset($booked_data) && $booked_data['selected_seats'] != '' ? $booked_data['selected_seats'] : null;?>"/>
									<input type="hidden" id="bs_selected_bus_<?php echo $index;?>" name="bus_id" value="<?php echo isset($booked_data) && $booked_data['bus_id'] != '' ? $booked_data['bus_id'] : null;?>"/>
									<input type="hidden" id="bs_has_map_<?php echo $index;?>" name="has_map" value="<?php echo isset($booked_data) ? $booked_data['has_map'] : null;?>"/>
									...
									<?php if (isset($tpl['return_bus_arr']) && !empty($tpl['return_bus_arr'])) { ?>
										...
										if (isset($STORE['is_return']) && $STORE['is_return'] == 'T')
										{
											...
											<div class="panel-body pjBsSeatsBody pjBsReturnSeatsBody" style="display:<?php echo isset($booked_data) && $booked_data['selected_ticket'] > 0 ? 'block' : 'none';?>;">
												...
												<div class="pjBsListSeats">
													<div class="pjBsChosenSeats" style="display:<?php echo isset($booked_data) && $booked_data['selected_ticket'] > 0 ? 'block' : 'none';?>;">
														<p><?php __('front_select');?> <strong id="bsReturnSeats_<?php echo $index;?>"><?php echo isset($booked_data) ? ( empty($intersect) ? ( $booked_data['return_selected_ticket'] > 0 ? ($booked_data['return_selected_ticket'] != 1 ? ($booked_data['return_selected_ticket'] . ' ' . pjSanitize::clean(__('front_seats', true, false))) : ($booked_data['return_selected_ticket'] . ' ' . pjSanitize::clean(__('front_seat', true, false))) ) :null) :null): null;?></strong></p>
				
														<dl class="dl-horizontal" style="display: <?php echo isset($booked_data) && $booked_data['selected_ticket'] > 0 ? 'inline-block' : 'none';?>;">
															<dt><?php __('front_selected_seats');?></dt>
															<dd id="bsReturnSelectedSeatsLabel_<?php echo $index;?>"><?php echo isset($booked_data) ? ( empty($intersect) ? ($booked_data['return_selected_seats'] != '' ? join(", ", $tpl['return_selected_seat_arr']) : null) : null ) : null;?></dd>
														</dl><!-- /.dl-horizontal -->
													...
													<div id="bsReturnMapContainer_<?php echo $index;?>" class="bsReturnMapContainer" style="display:<?php echo isset($booked_data) && $booked_data['return_has_map'] == 'T' ? 'block' : 'none';?>;">
														<?php
										...
										<input type="hidden" id="bs_return_selected_tickets_<?php echo $index;?>" name="return_selected_ticket" value="<?php echo isset($booked_data) && $booked_data['return_selected_ticket'] > 0 ? $booked_data['return_selected_ticket'] : null;?>" data-map="<?php echo isset($booked_data) && $booked_data['return_selected_ticket'] > 0 ? (!empty($tpl['bus_type_arr']['seats_map']) ? 'T' : 'F') : 'F';?>"/>
										<input type="hidden" id="bs_return_selected_seats_<?php echo $index;?>" name="return_selected_seats" value="<?php echo isset($booked_data) && $booked_data['return_selected_seats'] != '' ? $booked_data['return_selected_seats'] : null;?>"/>
										<input type="hidden" id="bs_return_selected_bus_<?php echo $index;?>" name="return_bus_id" value="<?php echo isset($booked_data) && $booked_data['return_bus_id'] != '' ? $booked_data['return_bus_id'] : null;?>"/>
										<input type="hidden" id="bs_return_has_map_<?php echo $index;?>" name="return_has_map" value="<?php echo isset($booked_data) ? $booked_data['has_map'] : null;?>"/>
									<?php } else { ?>
										<div class="panel-body"><?php __('front_no_bus_available');?></div>
									<?php } ?>
								</div><!-- /.panel panel-default pjBsSeats -->	
						...
						<footer class="pjBsFormFoot">
							<p class="text-right pjBsTotalPrice">
								<strong id="bsRoundtripPrice_<?php echo $index;?>"></strong>
							</p><!-- /.text-right pjBsTotalPrice -->
	
							<div class="clearfix pjBsFormActions">
								<a href="#" id="bsBtnCancel_<?php echo $index;?>" class="btn btn-default pull-left"><?php __('front_button_back'); ?></a>
								<?php
								...
								{ 
									?>
									<button type="button" id="bsBtnCheckout_<?php echo $index;?>" class="btn btn-primary pull-right"><?php __('front_button_checkout'); ?></button>
									<?php
								} 
								?>
